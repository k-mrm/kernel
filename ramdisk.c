...
static int ramdisk_read(struct block *dev, struct buf *buf) {
...
static int ramdisk_write(struct block *dev, struct buf *buf) {
  struct ramdisk *disk = container_of(dev, struct ramdisk, block);
  int offset;
...
static void ramdisk_suspend(struct device *dev) { ; }

static void ramdisk_resume(struct device *dev) { ; }

static int ramdisk_probe(struct device *dev) {
