...
static void pci_suspend(struct device *dev) { ; }

static void pci_resume(struct device *dev) { ; }
...
static struct pci_device *pcifind(u16 vendor, u16 device) { return NULL; }

static struct pci_driver *pci_find_driver(struct pci_device *pci) {
  struct pci_driver *drv;
  struct pci_id *id;

  LIST_FOREACH(drv, &pci_driver, dn) {
    for (id = drv->id; id->vendor != 0; id++) {
      if (pci->vendorid == id->vendor && pci->deviceid == id->device) {
        log("found driver: %s\n", drv->name);
        return drv;
      }
    }
...
int reg_pci_driver(struct pci_driver *drv) {
  list_add(&pci_driver, &drv->dn);
  return 0;
}

static char *pciname(struct pci_device *pci, char *name) {
  sprintf(name, "%02x:%02x.%02x", pci->bus, DEVNO(pci->devfn),
          FUNCNO(pci->devfn));
  return name;
}

static int new_pci(int bus, int devfn) {
...
static void initpci(void) {
  ...
  for (bn = 0; bn < 256; bn++)
    for (dn = 0; dn < 32; dn++)
      for (fn = 0; fn < 8; fn++) {
        pci_cfg_read(bn, DEVFN(dn, fn), PCI_CONFIG_VENDOR_ID, sizeof v,
                     (u32 *)&v);
        if (v == 0xffff)
          continue;
