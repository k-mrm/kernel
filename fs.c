...
void
delete_fs(struct fs_if *op)
{
        list_delete(&op->fn);
}

static struct fs_if *
identfs(struct block *dev)
{
        struct buf *sb;
        struct fs_if *fsif;
...
static const char *
skippath (const char *path, char *name, int *err)
{
        int len = 0;
...
static struct inode *
traverse(struct fs *fs, struct inode *pi, const char *path, char *name)
{
        int err = 0;
...
static void
regmountpoint(char *path, struct fs *fs)
{
        if (strcmp (path, "/") == 0)
                rootfs = fs;
        else
