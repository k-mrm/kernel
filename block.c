...
static struct buf *balloc(struct block *dev) {
  struct buf *buf;
...
static struct buf *bget(struct block *dev, int bno) {
  struct buf *buf;
...
struct buf *bootblock(struct block *dev) { return bread(dev, 0); }

struct buf *superblock(struct block *dev) { return bread(dev, 1); }
...
int probe_block(struct block *dev) {
  initlist(&dev->cache);
  blockdev = dev;
  return 0;
}
