...
static int apic_timer_freq(struct apic *apic) {
  u32 cnt, cnt2;
...
static int apictimer_probe(struct device *device) {
...
static void apictimer_resume(struct device *device) {
  struct eventtimer *et = dev_eventtimer(device);
  struct apic *apic = container_of(et, struct apic, timer);
  u32 lvt;
...
static void apictimer_suspend(struct device *device) {
  struct eventtimer *et = dev_eventtimer(device);
  struct apic *apic = container_of(et, struct apic, timer);
  u32 lvt;
...
static int apic_ack(struct irq *irq) { return -1; }

static void apic_eoi(struct irq *irq) {
...
static uint apictimergetperiod(struct eventtimer *et) {
  struct apic *apic = container_of(et, struct apic, timer);
  return apic->periodms;
}

static void apictimersetperiod(struct eventtimer *et, uint ms) {
  struct apic *apic = container_of(et, struct apic, timer);
  apic->periodms = ms;
}

static int apictimer_irq(struct eventtimer *et, struct irq *irq) { return 0; }

static void enable_apic(struct device *dev) {
...
static void disable_apic(struct device *dev) {
  struct irqchip *ic = dev_irqchip(dev);
  struct apic *apic = localapic;
  u32 spiv;
...
static void setspiv(struct apic *apic) {
  u32 spiv;
  spiv = 0x20; // sprious interrupt vector is 0x20
  apic->ops->write(apic, SPIV, spiv);
}

static int apic_enable_irq(struct irq *irq) { return 0; }

static int apic_disable_irq(struct irq *irq) { return 0; }

static int apic_probe(struct device *dev) {
...
static struct irq *apic_new_irq(struct irqchip *ic, struct device *dev,
...
void apicinit(u32 id, struct apic_if *ops) {
  ...
  rc = new_device(&apic->timer.dev, "eventtimer", "LAPICTimer", &apic_et_drv,
                  &cpu->devtree);
  if (rc < 0)
    return;
