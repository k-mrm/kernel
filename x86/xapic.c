...
static inline u32 xapicread32(u32 off) {
  return *(volatile u32 *)(apic_base + off);
}

static inline void xapicwrite32(u32 off, u32 val) {
  *(volatile u32 *)(apic_base + off) = val;
}

static u32 xapicrd(struct apic *apic, u32 reg) {
  u32 offset = reg;
  return xapicread32(offset);
}

static void xapicwr(struct apic *apic, u32 reg, u32 val) {
  u32 offset = reg;
...
static void xapicsendipi(struct apic *apic, int id) { ; }

static u32 apic_baseaddr(void) {
  ulong apicbase = rdmsr64(IA32_APIC_BASE);
  return (apicbase & IA32_APIC_BASE_APIC_BASE_MASK);
}

static int xapicprobe(struct device *dev, struct apic *apic) {
  struct iomem *iomem;
...
static bool xapicsupported(void) {
  u32 a, b, c, d;
