...
#define VENTRY(_n) asm(".quad __vector" #_n "_ \n")
...
static inline void loadidt(Gatedesc *idt, ulong idtsize) {
  volatile u16 t[5];
...
static inline void setgate(Gatedesc *desc, ulong offset, u16 sel, Gatetype type,
                           u8 dpl) {
...
void x86trapinit(void) {
  for (uint i = 0; i < NR_INTERRUPT; i++)
    setgate(idt + i, allvectors[i], SEG_KCODE << 3, GATEDESC_64_INTR,
            DPL_KERNEL);
  setgate(idt + 0x80, allvectors[0x80], SEG_KCODE << 3, GATEDESC_64_TRAP,
          DPL_USER);

  loadidt(idt, sizeof idt);
}

static void x86pagefault(struct trapframe *tf) {
  ulong faultaddr = cr2();
...
static void syscallint80(struct trapframe *tf) {
  tf->rax =
      (u64)syscall(tf->rax, (void *)tf->rdi, (void *)tf->rsi, (void *)tf->rdx,
...
struct proc *__cswitch(struct context *prev, struct context *next,
                       struct proc *pprev) {
  return pprev;
}
