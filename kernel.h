...
static inline void initlist(struct list *head) {
  head->next = head;
  head->prev = head;
}
...
static inline void list_add(struct list *h, struct list *e) {
  e->next = h->next;
  h->next->prev = e;
  e->prev = h;
  h->next = e;
...
static inline int list_len(struct list *h) {
  struct list *e;
  int i = 0;
...
static inline bool list_empty(struct list *h) { return list_len(h) == 0; }

static inline void list_delete(struct list *e) {
  struct list *n, *p;
...
static inline void inittree(struct tree *t) {
  t->par = NULL;
  initlist(&t->child);
}

static inline void new_child(struct tree *t, struct tree *c) {
  c->par = t;
  list_add(&t->child, &c->cn);
}

static inline void tree_node_delete(struct tree *n) { list_delete(&n->cn); }

static inline void *tree_dfs(struct tree *r, void *(*cb)(struct tree *, void *),
                             void *arg) {
  struct tree *t, *tmp;
  void *ret;
...
static inline int nchild(struct tree *t) { return list_len(&t->child); }
