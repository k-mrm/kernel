...
static PageBlock *newpageblock(void) {
  if (nproot < 32)
    return &proot[nproot++];
  else
    return NULL;
...
static Page *chunkdeletepa(FreeChunk *chunk, Phys pa) {
  Page *prev = NULL;
...
static void chunkaddpage(FreeChunk *chunk, Page *page) {
  page->next = chunk->freelist;
  chunk->freelist = page;
  chunk->nfree++;
}

static void splitpage(AllocBlock *kb, Page *p, uint order, uint blockorder) {
  FreeChunk *c;
  Page *latter;
  uint pagesize;
...
static Page *__allocpages(AllocBlock *kb, uint order) {
  FreeChunk *c;
  Page *p;
...
Page *allocpages(uint order) { return __allocpages(&kblock, order); }
...
static void mergepage(AllocBlock *kb, Page *page, uint order) {
  FreeChunk *chunk;
  ulong pfn, buddypfn;
  Page *buddy;
  Page *p0;
...
void freepages(Page *page, uint order) { mergepage(&kblock, page, order); }

static void initpageblock(MemBlock *block) {
  PageBlock *pb;
...
static uint earlyfreeblock(PageBlock *pb) {
  Page *page;
  Phys addr = pb->base;
  ...
  trace("early free block: %p-%p %d bytes\n", pb->base,
        pb->base + (pb->npages << PAGESHIFT), pb->npages << PAGESHIFT);

  for (ulong i = 0; i < pb->npages; i++, addr += PAGESIZE) {
    page = pb->pages + i;
    page->blockno = bno;
    // FIXME: naive
    if (!reservedaddr(addr, PAGESIZE)) {
      npages++;
      freepages(page, 0);
    }
  }
...
void kallocinitearly(ulong start, ulong end) {
  MemBlock *block;
  PageBlock *pblock;
  ulong npages = 0;
  ...
  FOREACH_SYSMEM_AVAIL_BLOCK(block) { initpageblock(block); }

  FOREACH_PAGEBLOCK(pblock) { npages += earlyfreeblock(pblock); }
